...
async function inspectViewport(width, height, label) {
  ...
  if (label === 'desktop') {
    ...
    const farEast = page.getByRole('button', { name: /Дальний Восток/i }).first();
    if (await farEast.count()) {
      await farEast.click();
      await page.waitForTimeout(250);
    }

    const map = page.locator('#logistics, .logistics-map').first();
    if (await map.count()) {
      await map.scrollIntoViewIfNeeded();
      await page.screenshot({ path: resolve(outputDirectory, 'standalone-map.png') });
    }
