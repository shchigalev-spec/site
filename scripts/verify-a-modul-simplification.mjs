...
async function verifyViewport(width, height) {
  ...
  const state = await page.evaluate(() => ({
    ...
    forbidden: ['.assembly__hud', '.assembly__stages', '.assembly__caption', '.hero__context', '.logistics', '.finder', '.factory', '.dominant-case']
...
try {
  ...
  const reducedState = await reduced.locator('.assembly').evaluate((node) => ({
    ...
    reveal: getComputedStyle(node.querySelector('.assembly__reveal')).clipPath
  }));
  if (!reducedState.reveal.includes('inset(0px)')) throw new Error(`Reduced motion hero is incomplete: ${JSON.stringify(reducedState)}`);
